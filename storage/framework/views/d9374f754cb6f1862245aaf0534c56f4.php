  ...
  <section>
    ...
    <?php if(!empty($judgeNames)): ?>
        <p style="margin-bottom: 20px; font-weight: bold;">
            القاضي: <?php echo e(implode(' ، ', $judgeNames)); ?>
    ...
    <div class="cases-grid">
        <?php $__empty_1 = true; $__currentLoopData = $cases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $case): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            ...
            <div class="case-strip">
                <div class="case-info">
                    <h3>القضية رقم: <?php echo e($case->number); ?></h3>
                    <p><strong>عنوان الدعوى:</strong> <?php echo e($case->type); ?></p>
                    
                    <?php if($session): ?>
                        <p><strong>تاريخ الجلسة:</strong> <?php echo e($session->session_date); ?></p>
                        <p><strong>حالة الجلسة:</strong> <?php echo e($session->status); ?></p>
                    <?php else: ?>
    ...
    <div class="case-actions">
        ...
        <?php if($session->status === 'محددة'): ?>
            <a href="<?php echo e(route('trial.report', ['session' => $session->id, 'source' => 'typist'])); ?>"
               class="action-btn">
               محضر المحاكمة
            </a>

        <?php elseif(in_array($session->status, ['مستمرة','مكتملة'])): ?>
            <a href="<?php echo e(route('trial.report', ['session' => $session->id, 'source' => 'typist'])); ?>"
               class="action-btn">
               محضر المحاكمة
            </a>

            <a href="<?php echo e(route('after.trial.report', ['session' => $session->id, 'source' => 'typist'])); ?>"
               class="action-btn">
               ما بعد
            </a>
        <?php endif; ?>
      ...
      <div class="modal-body">
        ...
        <div class="row mb-3">
          ...
          <div class="col-md-3">
            ...
            <input type="text" class="form-control form-control-sm" value="<?php echo e(date('Y')); ?>" readonly>
          </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const caseScheduleUrlTemplate = <?php echo json_encode(route('case.schedule', ['caseNumber' => 'CASE_NUMBER_PLACEHOLDER']), 512) ?>;
  ...
  <div class="modal-dialog modal-lg" style="margin-top: 80px;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="request-set-session-form" class="row g-3" method="POST" action="<?php echo e(route('typist.request.store-session')); ?>">
          <?php echo csrf_field(); ?>
  ...
  <div class="modal-dialog modal-lg" style="margin-top: 80px;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="request-reschedule-session-form" class="row g-3" method="POST" action="<?php echo e(route('typist.request.reschedule-session')); ?>">
          <?php echo csrf_field(); ?>
          <input type="hidden" name="id">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('delete_reschedule_session_button').addEventListener('click', function () {
    ...
    fetch(`<?php echo e(route('typist.request.delete-session')); ?>`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
  ...
  document.getElementById('save_reschedule_session_button').addEventListener('click', function () {
      ...
      fetch(`<?php echo e(route('typist.request.reschedule-session')); ?>`, {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
              "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('cancel-session-request-button').addEventListener('click', function () {
        ...
        fetch(`<?php echo e(route('typist.request.cancel-session')); ?>`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchRequestData(requestNumber) {
        fetch("<?php echo e(route('typist.judgment.open')); ?>?request_number=" + requestNumber)
        .then(response => response.json())
        .then(response => {
    ...
    document.getElementById('save_judgment').addEventListener('click', function () {
        ...
        fetch("<?php echo e(route('typist.judgment.store')); ?>", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
...
<?php /**PATH C:\Users\DELL\Desktop\legal_system3\resources\views/clerk_dashboard/typist.blade.php ENDPATH**/ ?>