...
<div class="court-bar"><?php echo e($judge->tribunal->name ?? '-'); ?> / <?php echo e($judge->department->name ?? '-'); ?></div>

<nav class="judge-bar">
  <div class="left-section">
    <span class="judge-name">القاضي/ <?php echo e($judge->full_name); ?></span>
    <ul class="nav-links">
      ...
      <table border="1" cellspacing="0" cellpadding="5">
        ...
        <tbody id="todaySessionsTable">
          <?php $__empty_1 = true; $__currentLoopData = $sessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr>
              <td><?php echo e($session->courtCase->number ?? '-'); ?></td>
              <td><?php echo e($session->courtCase->type ?? '-'); ?></td>
              <td><?php echo e($session->courtCase->created_at->format('Y-m-d')); ?></td>
              <td><?php echo e(\Carbon\Carbon::parse($session->session_date)->format('H:i')); ?></td>

              
              <td><?php echo e($session->judgment_type ?: 'لم يصدر حكم'); ?></td>

              
              <td><?php echo e($session->status); ?></td>

              
              <td><?php echo e($session->session_goal ?: 'لم يتم تحديد سبب الجلسة'); ?></td>
            </tr>
    ...
    <table border="1" cellspacing="0" cellpadding="5">
      ...
      <tbody id="casesTable">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $cases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $case): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <?php $__currentLoopData = $case->participants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $participant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            ...
            ?>
            <tr>
              <?php if($isFirstParticipant): ?>
                <td rowspan="<?php echo e($rowspan); ?>"><?php echo e($case->number); ?></td>
                <td rowspan="<?php echo e($rowspan); ?>"><?php echo e($case->type); ?></td>
              <?php endif; ?>
              <td>طرف <?php echo e($index + 1); ?> - <?php echo e($participant->type); ?></td>
              <td><?php echo e($participant->name); ?></td>
              <td><?php echo e($participant->charge); ?></td>
              <td><?php echo e($memo->detention_duration ?? '-'); ?></td>
              <td><?php echo e($memo->detention_reason ?? '-'); ?></td>
              <td><?php echo e($memo->released ?? '-'); ?></td>
              <td><?php echo e($memo->detention_center ?? '-'); ?></td>
              <td><?php echo e($notification->method ?? '-'); ?></td>
              <td><?php echo e($notification && $notification->notified_at ? \Carbon\Carbon::parse($notification->notified_at)->format('Y-m-d') : '-'); ?></td>
              <?php if($isFirstParticipant): ?>
                <td rowspan="<?php echo e($rowspan); ?>">
                  <div class="case-actions">
                    <?php if($firstSession): ?>
                      <?php if(\App\Models\CourtSessionReport::where('case_session_id', $firstSession->id)->where('report_mode','trial')->exists()): ?>
                        <a href="<?php echo e(route('judge.trial.report', $firstSession->id)); ?>" class="btn action-btn">محضر المحاكمة</a>
                      <?php endif; ?>
                      <?php if(\App\Models\CourtSessionReport::where('case_session_id', $firstSession->id)->where('report_mode','after')->exists()): ?>
                        <a href="<?php echo e(route('judge.after.report', $firstSession->id)); ?>" class="btn action-btn">ما بعد</a>
                      <?php endif; ?>
                      <?php if(!\App\Models\CourtSessionReport::where('case_session_id',$firstSession->id)->exists()): ?>
                ...
                <td rowspan="<?php echo e($rowspan); ?>"><?php echo e($case->created_at ? $case->created_at->format('Y-m-d') : '-'); ?></td>
                <td rowspan="<?php echo e($rowspan); ?>"><?php echo e($firstSession ? \Carbon\Carbon::parse($firstSession->session_date)->format('Y-m-d H:i') : '-'); ?></td>
              <?php endif; ?>
            </tr>
...
async function loadTodayRequests() {
    ...
    try {
        console.log("🔹 Calling: <?php echo e(route('judge.requests.today')); ?>");

        const response = await axios.get("<?php echo e(route('judge.requests.today')); ?>");
...
async function loadAllRequests() {
    ...
    try {
        console.log("🔹 Calling: <?php echo e(route('judge.requests.all')); ?>");

        const response = await axios.get("<?php echo e(route('judge.requests.all')); ?>");
...
</html><?php /**PATH C:\Users\LENOVO\legal_system\resources\views/clerk_dashboard/judge.blade.php ENDPATH**/ ?>