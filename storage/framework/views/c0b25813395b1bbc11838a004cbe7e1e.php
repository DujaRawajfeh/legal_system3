...
<?php $__env->startSection('title', 'لوحة الكاتب'); ?>
          ...
          <form id="caseForm">
            ...
            <div class="row g-3 mb-4">
              ...
              <div class="col-md-3">
                ...
                <input type="text" class="form-control" id="courtNumber" value="<?php echo e(auth()->user()->tribunal->number); ?>" readonly>
              </div>
              <div class="col-md-3">
                <label class="form-label">رقم القلم</label>
                <input type="text" class="form-control" id="departmentNumber" value="<?php echo e(auth()->user()->department->number); ?>" readonly>
              </div>
              <div class="col-md-3">
                <label class="form-label">السنة</label>
                <input type="text" class="form-control" id="caseYear" value="<?php echo e(date('Y')); ?>" readonly>
              </div>
            </div>
      ...
      <div class="modal-body">
        <div class="form-container">
          <form id="requestForm">
            ...
            <!-- موعد الجلسة -->
            <div class="section-title">موعد الجلسة</div>
            <div class="row g-3">
              <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="prosecutor_office" class="form-select">
                            ...
                            <?php $__currentLoopData = $records; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($rec->records); ?>"><?php echo e($rec->records); ?></option>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (notifyBtn) {
    ...
    notifyBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await fetch("<?php echo e(route('notifications.save')); ?>", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  caseSerial.addEventListener('keypress', function(e) {
    ...
    courtNumber.value = '<?php echo e(auth()->user()->tribunal->number); ?>';
    penNumber.value = '<?php echo e(auth()->user()->department->number); ?>';
  ...
  if (notifyBtn) {
    ...
    notifyBtn.addEventListener('click', () => {
      ...
      fetch("<?php echo e(route('notifications.save')); ?>", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
...
async function loadReportsList() {
    ...
    try {
        ...
        const afterUrlTemplate =
            `<?php echo e(route('writer.after-trial.report.show', ['session' => 'SESSION_ID'])); ?>?source=writer`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (openBtn && modalEl) {
        openBtn.addEventListener("click", () => {
            ...
            document.getElementById("reqCourtNumber").value = "<?php echo e(auth()->user()->tribunal->number); ?>";
            document.getElementById("reqDepartmentNumber").value = "<?php echo e(auth()->user()->department->number); ?>";
